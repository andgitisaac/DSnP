...
// it have to reg cmd in DB, see initCommonCmd() in cndCommon.cpp 
bool
initDbCmd()
{
   // TODO...
...
DBAppendCmd::exec(const string& option)
{
   ...
   bool doRow = false;
   if(myStrNCmp("-Rows", options[0], 2) == 0) doRow = true;
   ...
   // Add data 
   // If fewer data is provided (but cannot be none), the missing cells will be
   // assumed “null” (i.e. empty). If more data is given, the extra ones will 
   // be ignored. There will be NO warning or error message for the above two cases.

   if(doRow){
      if(data.size() < dbtbl.nCols()){
         size_t length = dbtbl.nCols() - data.size();
         for(size_t i = 0; i < length; i++) data.push_back(INT_MAX);
      }
      else if(data.size() > dbtbl.nCols()){
         size_t length = data.size() - dbtbl.nCols();
         for(size_t i = 0; i < length; i++) data.pop_back();
      }
      DBRow row(data);
      dbtbl.addRow(row);
   }
      ...
      if(data.size() < dbtbl.nRows()){
         size_t length = dbtbl.nRows() - data.size();
         for(size_t i = 0; i < length; i++) data.push_back(INT_MAX);
      }
      else if(data.size() > dbtbl.nRows()){
         size_t length = data.size() - dbtbl.nRows();
         for(size_t i = 0; i < length; i++) data.pop_back();
      }
      dbtbl.addCol(data);
   }
   ...
   if(myStrNCmp("-Table", options[0], 2) == 0){
      ...
      if(checkRowIdx(options[0], row)){
         ...
         if(checkColIdx(options[1], col)){
            if(dbtbl[row][col] != INT_MAX) cout << dbtbl[row][col] << endl;
            else cout << "null" << endl;
         }
      }
   }
