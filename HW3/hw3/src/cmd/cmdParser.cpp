...
bool ltrim(string const&, string&, size_t&);
//----------------------------------------------------------------------
//    External funcitons
//----------------------------------------------------------------------
void mybeep();
...
{
   // TODO...
   // First, save the current _dofile in stack, then point the _dofile
   // to the new file.
   // If failed to open the new file(e.g. file not exist), the memory
   // of _dofile point to must be destory and re-point to the top of
   // stack. Remember to pop away the top of stack since the current
   // file is reading.
   if(!(_dofile)) _dofile = new ifstream(dof.c_str());
   else{
...
CmdParser::regCmd(const string& cmd, unsigned nCmp, CmdExec* e)
{
   ...
   // e.g. cmd = HELp or HEL, nCmp = 3, e = HelpCmd
   // getCmd(cmd) return CmdExec* e, now e = HELp(the entire command)
   string str = cmd;
   unsigned s = str.size();
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
   // NOTE: it's called by "CmdExec* e = parseCmd(option) 
   //       in CmdParser::execOneCmd() 
   string cmd;
   size_t optCmdBeginIdx = myStrGetTok(str, cmd);
   // WTF means return 'it' at the end?????????
   // getCmd(cmd) return CmdExec* e, now e = HELp(the entire command)
   ...
   (optCmdBeginIdx != string::npos) ? option = str.substr(optCmdBeginIdx)
                                    : option = "";
...
CmdParser::listCmd(const string& str)
{
   ...
   // list all command for null cmd or cmd with only ' '
   if(!(ltrim(str, cmd, pos))){
      int cnt = 1;
      CmdMap::const_iterator it = _cmdMap.begin();
      cout << endl;
      for( ; it != _cmdMap.end(); ++it){
         string wholeCmd = it->first + (it->second->getOptCmd());
         if(cnt % 5) cout << setw(12) << left << wholeCmd;
         else cout << setw(12) << left << wholeCmd << endl;
         cnt++;
      }
      reprintCmd();
   }
   ...
   // partially match and no match
   else{
      vector<string> candidate;
      CmdMap::const_iterator it = _cmdMap.begin();
      for( ; it != _cmdMap.end(); ++ it){
         const string wholeCmd = it->first + (it->second->getOptCmd());
         // NOTE : when wholeCmd.size < cmd.size will raise assertion fail
         // in myStrNCmp QQ
         if(wholeCmd.size() < cmd.size()) continue;
         if(myStrNCmp(wholeCmd, cmd, cmd.size()) == 0) 
            candidate.push_back(wholeCmd);
      }
      if(candidate.size() == 1){
         // cmd> he$hahah
         // [After Tab]
         // cmd> heLp $hahaha 
         for(size_t i = str.size(); i < candidate[0].size(); i++)
            insertChar(candidate[0][i]);
         insertChar(' ');
      }
      else if(candidate.empty()){ mybeep(); }
      else{     
         cout << endl;
         for(size_t i = 0; i < candidate.size(); i++){
            if((i + 1) % 5) cout << setw(12) << left << candidate[i];
            else cout << setw(12) << left << candidate[i] << endl;
         }
         reprintCmd();
      }
   }
...
{
   // e.g. cmd = HELp or HEL, e will final become HelpCmd
   CmdExec* e = 0;
   // TODO...
   // iterator->first is the mandatory part of the command string
   // (e.g. HEL in Help)
   // iterator->second is the complete name of cmd, access the optional 
   // part by calling getOptcmd in class CmdExec
   // typedef map<const string, CmdExec*>   CmdMap; (in cmdParser.h)
   // iterator->second return e, not *e....
   CmdMap::iterator it = _cmdMap.begin();
   for( ; it != _cmdMap.end(); ++it){
      const string mandCmd = it->first;
      const string wholeCmd = mandCmd + (it->second->getOptCmd());
      if(myStrNCmp(wholeCmd, cmd, mandCmd.size()) == 0) e = it->second;
   }
   return e; // getCmd(cmd) return the entire command
}
...
// Trimming left white space, return the trimmed token and end position.
// Return false when str is empty or space-only
bool ltrim(string const& str, string& cmd, size_t& pos){
