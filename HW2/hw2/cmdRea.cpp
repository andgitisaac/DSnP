...
CmdParser::readCmdInt(istream& istr)
{
   ...
   while (1) {
      ...
      switch (pch) {
         ...
         case TAB_KEY        : for(int i = ((_readBufPtr - _readBuf)%8); i < TAB_POSITION; i++) insertChar(' ');
                               break;
...
CmdParser::deleteChar()
{
  ...
  cout << " ";
  for(int i = 0; i <= _readBufEnd - _readBufPtr; i++) cout << char(8);
  return true;
}
...
CmdParser::insertChar(char ch, int repeat)
{
  ...
  _readBufPtr += repeat;
  _readBufEnd += repeat;
  for(int i = -repeat; i < _readBufEnd - _readBufPtr; i++) cout << *(_readBufPtr + i);
  for(int i = 0; i < _readBufEnd - _readBufPtr; i++) cout << char(8);
  assert(repeat >= 1);
}
...
CmdParser::deleteLine()
{
  ...
  for(int i = 0; i < length; i++) deleteChar();
}
...
CmdParser::moveToHistory(int index)
{
  ...
  if(index > (_history.size() - _tempCmdStored)) {
    if(_historyIdx == (_history.size() - _tempCmdStored)) mybeep();
    else moveToHistory(_history.size() - _tempCmdStored);
    return;
  }
  if(_historyIdx + _tempCmdStored == _history.size()) {
    if(_tempCmdStored) _history.pop_back();
    string temp(_readBuf, _readBufEnd - _readBuf);
...
{
  if(_tempCmdStored) _history.pop_back();
  string add(_readBuf, _readBufEnd - _readBuf);
  int s = 0, e = add.length() - 1;
  while(1) {
    if(add[s] == ' ') s++;
    else break;
  }
  ...
  if(e == -1) return;
  _history.push_back(add.substr(s, e - s + 1));
