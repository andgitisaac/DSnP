...
unsigned CirGate::validAig(vector<string>* i_gate, vector<string>* name, vector<string>* aig) const {
    ...
    if(_type == PI_GATE){
        ...
        unsigned g1 = getFanin(0)->validAig(i_gate, name, aig),
...
CirGate::reportGate() const
{
    ...
    ss << "= FECs:";
    if(_fecGrp){
        FECGroup* fec = (FECGroup*)(_fecGrp & ~(size_t)(0x1));
        bool inv = _fecGrp & 1;
        for(FECGroup::iterator iter = fec->begin(); iter != fec->end(); ++iter){
            if((*iter) != this) ss << ((iter.isInv() ^ inv) ? "!" : "")
                                    << (*iter)->getGateId() << " ";
        }
    }
    str = ss.str();
