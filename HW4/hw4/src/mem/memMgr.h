...
#define toSizeT(t)  ((t % SIZE_T) ? (SIZE_T * (t / SIZE_T + 1)) : (SIZE_T * (t / SIZE_T))) // TODO
//
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      size_t properSize = toSizeT(t);
      if(properSize > getRemainSize()) return false;
      ret = (T*)_ptr;
      _ptr += properSize;
      return true;
   }
...
class MemRecycleList
{
   ...
   T* popFront() {
      ...
      // and then point _first to the next one. 
      if(!(_first)) return 0;
      T* p = _first;
      _first = getNext(p);
      return p;
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      MemBlock<T>* block = _activeBlock->getNextBlock();
      while(block){
         delete _activeBlock;
         _activeBlock = block;
         block = _activeBlock->getNextBlock();
      }
      _activeBlock->reset();
      for(int i = 0; i < R_SIZE; ++i) _recycleList[i].reset();
      if(!( (b == 0) || (b == _blockSize) )){
         _blockSize = b;
         delete _activeBlock;
         _activeBlock = new MemBlock<T>(0, _blockSize);
...
private:
   ...
   // [Note] t must >= S #define S sizeof(T)
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* ll = &(_recycleList[m]);
      while(true){
         if(ll->getArrSize() == n) return ll;

         if(!(ll->getNextList())){
            ll->setNextList(new MemRecycleList<T>(n));
            ll = ll->getNextList();
            return ll;
         }
         else ll = ll->getNextList();
      }
   }
   ...
   T* getMem(size_t t) {
      ...
      // Check whether the remain memory is enough...
      // bool getMem(size_t t, T*& ret) in template <class T> class MemBlock
